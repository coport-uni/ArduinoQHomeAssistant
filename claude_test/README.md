...
| `toggle_test.sh` | Toggles a HA switch entity on/off at 3 s intervals for N cycles, verifying reported state after each command. Usage: `toggle_test.sh <entity_id> <cycles>`. Run on the board. | `switch.tapo_p1` (052F, .239) passed 6/6 transitions; state propagates to HA within ~1 s of the service call. |
...
adb shell 'docker run --rm --network=host -v /tmp/probe_all.py:/probe.py \
